...
#This cleans column names
  #The original column names had inconsistent capitalization—some included spaces and others did not—so this feature standardizes the format of all column names.
SF <- janitor::clean_names(SF)
#This processes dates for analysis
  #mdy is transferring the date to year-month-day
...
ggplot(Years,aes(x = year, y = n,
             fill = n)) +
  geom_col(position = "dodge") + #this is to give some space between bars
...
**1) Which Day Had the Most 311 Calls?** Using count to tabulate calls by day
...
```{r}
help("slice_max")
slice_max(Days, order_by = n, n = 5)

#or
Days %>%
  slice_max(order_by = n, n = 5) #order_by can be removed. like slice_max(n, n = 5)

#the top 5 days with the most calls are 2019-08-15, 2018-04-19, 2016-04-18, 2018-03-28, 2018-05-08
```
...
```{r}
slice_min(Days, order_by = n, n = 5)
#or
Days %>%
  slice_min(order_by = n, n = 5)

#the top 5 days with the fewest calls are 2016-03-31, 2019-06-30, 2018-06-09, 2018-08-18, 2019-01-06
```
...
#so the common days for arrests are Tuesday, Wednesday, and Thursday (the numbers are close to each other)
