...
```{r}
MediaBucks %>% 
  count(Gender) %>% #count the unique values of one or more variables. count(a,b,...)
  arrange(desc(n))
```
...
MediaBucks %>% 
  select(Gender, Salary) %>%
  group_by(Gender) %>%
  summarise(Total = sum(Salary, na.rm=TRUE), Avg_Salary = mean(Salary, na.rm=TRUE))
...
#Quick filter out hourly workers codes:
MediaHourlyWorkers <- MediaBucks %>%
  filter(Salary_Details == "hour")
```
...
range(MediaBucks$Salary, na.rm = T)

#number of rows
nrow(MediaBucks)

#number of rows cut with filter of greater than 500
nrow(MediaBucks %>%
  filter(Salary >= 500))
...
#grepl(): search for meatches to argument pattern within each element of a character vector
...
```{r}
#3: Identify title, gender and race of the highest paid position at NYT  
order(NYT$Salary, decreasing = T)
#or
which.max(NYT$Salary)
NYT[28, c("TITLE", "Gender", "Race")]

#in case there are multiple highest paid persons
NYT[NYT$Salary == max(NYT$Salary, na.rm = TRUE), c("TITLE", "Gender", "Race")]
...
#Which one is higher?
#based on the summary table, Bloomberg won.
...
Reporters[Reporters$Salary == 230504, ]
...
order(Editors$Salary, decreasing = T)
which.max(Editors$Salary)
Editors[122, ]
#or
Editors[Editors$Salary == max(Editors$Salary, na.rm = TRUE), ]

#make a table for any position involving data
unique(MediaBucks$TITLE)
Dataposition <- subset(MediaBucks, grepl("data|Data", TITLE))
